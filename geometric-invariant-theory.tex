...
\begin{proof}
Let $A:=\mathcal{O}(X) \supset \mathcal{O}(X)^G:=B$. Then the coordinate ring of
$X$ is a finitely presented algebra
$\mathcal{O}(X)=k[f_1,\ldots,f_n]$. For every generator of  $\mathcal{O}(X)$
define a polynomial 
$P_i(t):=\prod_{g \in G}(t-gf_i) \in A^G[t]=B[t]$.
\end{proof}
